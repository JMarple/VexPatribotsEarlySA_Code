...
void crossOverTwo()
{
  ...
  for(int y = 0; y < pathToGen - 1; y++)
  {
    ...
    {
      //Find two potential targets, except that targetA is always the same one
      byte targetA = extractNum(parents[y+1].path, x);
      byte targetB = extractNum(parents[y].path,x);

      //Find the tile to compare the targets to
      ...
      if(mutation<mutationPercentage)
      {
        ...
        //Is targetA a better mathc then targetB?
        if(disA < disB)
        {
          ...
          //If targetA isn't valid, check targetB
